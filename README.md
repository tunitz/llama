...
```sh
llama serve --models-preset ./preset.ini
```
...
```sh
llama serve --models-preset ./preset.ini --temp 0.1
```
...
```sh
# Different port
llama serve --models-preset ./preset.ini --port 9090

# Bind to all IPv4 interfaces (reachable from other machines on the network)
llama serve --models-preset ./preset.ini --host 0.0.0.0 --port 9090
```
...
```sh
ln -s /path/to/llama/models.json ~/.pi/agent/models.json
```
